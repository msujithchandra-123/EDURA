...
const WorksheetCard: React.FC<{ assignment: Assignment, status: SubmissionStatus }> = ({ assignment, status }) => {
    ...
    return (
        <Card>
            ...
            <div className="mt-4">
                <Link to={`/student/worksheets/${assignment.id}`}>
                    <Button size="sm">
                        {status === SubmissionStatus.PENDING ? 'Start Worksheet' : 'View Details'}
                    </Button>
                </Link>
            </div>
        </Card>
...
const StudentWorksheet: React.FC = () => {
    ...
    useEffect(() => {
        const fetchWorksheets = async () => {
            try {
                setLoading(true);
                const fetchedAssignments = await api.getStudentAssignments('worksheet');
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-6">Worksheets</h1>
            {assignments.length === 0 ? (
                <p className="text-gray-500">No worksheets found.</p>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {assignments.map(assignment => (
                        <WorksheetCard key={assignment.id} assignment={assignment} status={statuses[assignment.id] || SubmissionStatus.PENDING}/>
                    ))}
                </div>
            )}
        </div>
